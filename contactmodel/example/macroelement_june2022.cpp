/** \file contactmodelexample.cpp
  ...
  * @{
*/
#include "macroelement_june2022.h"
...
namespace cmodelsxd {
	...
	//		h = 1.0;
		...
		if (F < 0) {
			...
			double MYD = MY / D;
			//f = F * F / me_fy_ / me_fy_ + MD * MD / MYD / MYD - 1.0;
...
/*		if (myStatus.moment_ < 0) {
			myStatus.moment_ *= -1.0;
			myStatus.moment_sign_ *=-1;
		}
*/
//		else
//			myStatus.moment_sign_ = 1;
	...
	//		 double f = yieldf(curr_state.u_p_, curr_state.omega_p_, curr_state.force_, curr_state.moment_);
			//double temp_f = f;
		//	if (f > 1.0e-2)
			//	int dog = 1;
	...
	//	f = yieldf(curr_state.u_p_, curr_state.omega_p_, curr_state.force_, curr_state.moment_);
			//double temp_f = f;
			//if (f > 1.0e-2)
			//	int dog = 1;
	...
	ContactModelmacroelement::epIncrements ContactModelmacroelement::evaluatePlasticity(systemStatus myState, dfIncrements dfinc) {
		...
		// wire parameters
		double r = 1.35e-3; // need to consider actual element size
		double L = myState.gap_ + 2.0 * r;
		me_curr_l(L);

		
		// get the plastic strains
		...
		//	double h = exp(-1.0 / me_beta_ * pow(-0.5 + F_star / nf,2));
		...
		if (F < 0) {
			...
			//cos_theta = cos(atan(tan_theta)*100);
				//sin_theta = sin(atan(tan_theta) * 100);
				//sin_theta = 1.0 - sin_theta;
				//df_dF = 2 * F / me_fy_ / me_fy_;
				//df_dM = 2 * MD / MYD / MYD;
			df_dF = 2 * F * MY / (D * me_ntens_ * me_ntens_);
			df_dM = 2 * MD / MY;
			//double tmp = M - MY + me_fres_ * sin_theta;
			//double df_dmy = 2 * tmp * (me_fres_ * cos_theta / me_fy_ - 1) / (me_fres_ * me_fres_ * sin_theta * sin_theta) + 2 * F * F * sin_theta * sin_theta / (me_fy_ * me_fres_ * me_fres_ * cos_theta * cos_theta * cos_theta) - 2 * tmp * cos_theta * cos_theta / (me_fy_ * me_fres_ * me_fres_ * sin_theta * sin_theta * sin_theta);
			//double df_dmy = -2.0 * MD * MD / MYD /MYD / MYD;
...
//			 temp1 = exp(-1.0 / me_betag_ * pow(F_star / nf - me_alphag_, 2));
//			 temp2 = F - nf / 2;
		...
		//	df_dM = df_dM * sign_trial_M;
		//	dg_dM = dg_dM * sign_trial_M;




		double nomin = df_dM * kEI * omega_dot + df_dF * kEA * u_dot; // (me_curr_l_ ) * u_dot;
		double denom1 = -1.0 * df_dnf * (dnf_domegap * (dg_dM)+dnf_dup * dg_dF); // the part due to plastic increment
		double denom2 = df_dM * dg_dM * kEI +df_dF * dg_dF * kEA; // (me_curr_l_); //the part due to elastic increment
		...
		myIncrements.force_dot_ = (u_dot - lambda * dg_dF) * kEA;// (me_curr_l_);
		myIncrements.moment_dot_ = (omega_dot - lambda * dg_dM) * kEI;// (me_curr_l_ );

		double nfdot = dnf_dup * abs(myIncrements.u_p_dot_) + dnf_domegap * abs(myIncrements.omega_p_dot_);

		double temp = (u_dot - lambda * dg_dF)* kEA / (me_curr_l_ )/ (dnf_dup * abs(myIncrements.u_p_dot_) + dnf_domegap * abs(myIncrements.omega_p_dot_));

		// calculating nfdot this way should reduce the rounding errors and improve the overall yieldf fitting
		...
		if (isnan(nfdot))
			throw Exception("nanananananananana");

		/*
		double ftrial = yieldf(myState.u_p_ + myIncrements.u_p_dot_, myState.omega_p_ + myIncrements.omega_p_dot_, F + myIncrements.force_dot_, M + myIncrements.moment_dot_);

		double old_ftrial = ftrial;
		...
		// bring the stress increments back to the yieldf using bisection
		a = (a0 + a1) / 2.0;



		ftrial = yieldf(myState.u_p_ + myIncrements.u_p_dot_*(a), myState.omega_p_ + myIncrements.omega_p_dot_ * (a), F + myIncrements.force_dot_ * a, M + myIncrements.moment_dot_ * a);
		...
		*/
		myIncrements.force_dot_ = myIncrements.force_dot_;// *a;
		myIncrements.moment_dot_ = myIncrements.moment_dot_;// *a;
		myIncrements.u_p_dot_ = myIncrements.u_p_dot_;// *(2.0 - a);
		myIncrements.omega_p_dot_ = myIncrements.omega_p_dot_;// *(2.0 - a);
		/*
		f = yieldf(myState.u_p_ + myIncrements.u_p_dot_, myState.omega_p_ + myIncrements.omega_p_dot_, F + myIncrements.force_dot_ , M + myIncrements.moment_dot_);
		...
		//double shear_modulus = me_ea_ / sa / (2.0 * (1.0 + me_poisson_));
		...
		//double d_omega_tot = vec_d_omega_tot[1];
		...
		double d_omega = d_omega_tot;
		 ...
		 double kEA = calculateElasticModulus(u_el_);// / (current_length + d_u);
		 double kEI = me_ei_ * kEA / me_ea_;
		//while (abs(d_u)  < abs(d_u_tot*(1.0- increment_size_tolerance)) || abs(d_omega)  < abs(d_omega_tot * (1.0 - increment_size_tolerance)) || iteration == 1) {
...
//					 u_el_ = (current_length - me_ref_l_) - me_u_p_;
//					double kEA2 = calculateElasticModulus(u_el_) / (current_length + d_u);
//					kEA = calculateElasticModulus((current_length  - me_ref_l_) - me_u_p_) / (current_length );
//					kEI = me_ei_ * kEA / me_ea_;
		...
		if (wp_force < 0)
			wp_force = 0;
		//omegap_increment * (updated_state.moment_ + initial_state.moment_) / 2 + 
		double wp_moment = omegap_increment * (updated_state.moment_ + initial_state.moment_) / 2;
		double delta_wp = wp_force + wp_moment;// + wp_force;
		me_wp_ += delta_wp;
		...
		if (isnan(damage_multiplier))
			damage_multiplier = 0.0;// throw Exception("nanananananananana");
		...
		// set a the contact force to the post-damage value
		//oops the value has to be negative
		//pbProps_->pb_F_ = (vec_force + force_inc) * -damage_multiplier; // (pbProps_->pb_F_ - force_inc)* damage_multiplier;
		//pbProps_->pb_M_ = (vec_moment + moment_inc) * -damage_multiplier;
		pbProps_->pb_F_ -= force_inc;
		pbProps_->pb_M_ -= moment_inc;

		// store the current force to pre-damage and post-damage
		me_force(updated_state.force_);
		me_moment(updated_state.moment_);
	...
	//	f = yieldf(updated_state.u_p_, updated_state.omega_p_, updated_state.force_, updated_state.moment_);

	//	f = yieldf(me_u_p(), me_omega_p(), pbProps_->pb_F_.x(), pbProps_->pb_M_.x());
	...
	bool ContactModelmacroelement::forceDisplacementLaw(ContactModelMechanicalState *state, const double &timestep) {
		...
		if (damage_multiplier <= 0.01) {
		//if ((updated_state.nf_ > me_nult_ * 2 * 0.99)&&(updated_state.force_ > 0)) {
