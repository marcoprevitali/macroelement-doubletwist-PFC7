...
namespace cmodelsxd {
	...
	public:
		...
		struct systemStatus {     // struct containing the plastic variables / hardening
			double u_p_;          // plastic displacement
			double omega_p_;      // plastic rotation
			double force_;        // axial force
			double moment_;       // bending moment
			double u_slide_;      // wire sliding displacement
			double wp_;           // plastic work
			double nf_;           // yield surface size
			double gap_;          // reference particle-particle gap
			int moment_sign_ = 1; // variable to track the moment sign to have the model work for any bending direction
		};

		struct epIncrements {     // elasto-plastic increments in hardening variables and generalises forces
			double force_dot_;	  // axial force increment
			double moment_dot_;   // bending moment increment
			double u_p_dot_;      // plastic displacement increment
			double omega_p_dot_;  // plastic rotation increment
			double nf_dot_;       // yield surface size increment
			double wp_;           // plastic work increment
		};

		struct dfIncrements {     // total generalised displacement increments
			double u_dot_;        // total displacement increment
			double omega_dot_;    // total rotation increment
		};

		// functions used to implement the macroelement
		...
		// not used, part of the thermal coupling model in PFC
		...
		// added the macroelement energies to the tracking
